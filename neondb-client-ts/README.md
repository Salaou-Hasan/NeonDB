...
TypeScript/JavaScript client SDK for [NeonDB](../) — the self-hosted, zero-cost real-time game backend.

**100% free.** No cloud account. No fees. NeonDB and Dokploy run on hardware you own.
...
```bash
npm install @neondb/client @msgpack/msgpack
...
const client = new NeonDBClient({
  ...
  // apiKey: "your-api-key",  // only if NEONDB_API_KEY is set server-side
});
...
NeonDB uses **MessagePack** (via `rmp_serde` on the server) with these conventions:
